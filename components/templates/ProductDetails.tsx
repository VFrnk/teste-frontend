...
export const ProductDetails = ({ product }: ProductDetailsProps) => {
  const registeredAt = new Date(product.date);
  const dateFullLabel = Number.isNaN(registeredAt.getTime())
    ? product.date
    : registeredAt.toLocaleDateString("pt-BR", { dateStyle: "full" });
      ...
      <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-foreground">
              {Number.isNaN(registeredAt.getTime())
                ? product.date
                : registeredAt.toLocaleDateString("pt-BR")}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4">
          ...
          <p className="text-foreground/70 leading-relaxed">
            ...
            <strong>{product.category}</strong> e foi registrado no sistema em{" "}
            {dateFullLabel}.
          </p>
        </div>
      </div>
